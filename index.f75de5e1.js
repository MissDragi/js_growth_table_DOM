...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    addColumnBtn.addEventListener("click", function(ev) {
        for (const row of table.rows)if (row.cells.length < 10) row.insertCell();
        updateButtonStates();
    });
    deleteColumnBtn.addEventListener("click", function(ev) {
        const canDelete = Array.from(table.rows).every((row)=>row.cells.length > 2);
        if (canDelete) for (const row of table.rows)row.deleteCell(-1);
        updateButtonStates();
    });
    updateButtonStates();
});
